...
export default function ScanUploadDialog(props) {
  ...
  const schema = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(AsyncUploadScan(values.patientID._id, formData, handleClose));
    },
  });
  ...
  useEffect(() => {
    ...
    schema.setFieldValue(
      "patientID",
      users.length !== 0
        ? users.filter((element) => element.role === "Patient")[0]
        : ""
    );
  }, [open]);
    ...
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
      <Box component="form" onSubmit={schema.handleSubmit}>
                ...
                >
                  ...
                  >
                    ...
                    <Button
                      variant="outlined"
                      onClick={() =>
                        document.getElementById("scan-document")?.click()
                      }
                    >
                      {t("Select Document")}
                    </Button>
                    {schema.values.file && (
                      <Typography
                        sx={{ fontSize: "0.75rem", color: "#059669", mt: 1 }}
                      >
        ...
        <DialogActions>
          ...
          <Button disabled={!schema.isValid} type="submit">
            {t("Save")}
          </Button>
        </DialogActions>
      </Box>
