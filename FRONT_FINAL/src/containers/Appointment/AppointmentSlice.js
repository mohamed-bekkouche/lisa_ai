...
export function AsyncAdminApproveAppointment(id) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.put(`/admin/appointments/approve/${id}`);
      const data = {
        id: id,
        status: "Approved",
      };
...
export function AsyncAdminRefuseAppointment(id) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.put(`/admin/appointments/refuse/${id}`);
      const data = {
        id: id,
        status: "Refused",
      };
...
export function AsyncDeleteAppointment(id) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.delete(`/patient/cancel-appointment/${id}`);
      const data = {
        id: id,
        status: "Cancelled",
      };
...
export function AsyncEditAppointment(id, role, date, time, status) {
  return async (dispatch) => {
    dispatch(editing());
    ...
    try {
      if (role === "Admin") {
        const response = await axios.put(
          `/admin/appointments/reschedule/${id}`,
          {
            date,
            time,
            status,
...
export function AsyncCreateAppointment(date, time) {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.post("/patient/take-appointment", {
        date,
        time,
      });
