...
export default function ScanResult() {
  ...
  const { name, role, id, email } = useSelector((state) => state.login);
  ...
  const handleDownload = async (scanResult) => {
    try {
  ...
  const generatePDFReport = (scanResult) => {
    const doc = new jsPDF();

    // Add logo or header
    doc.setFontSize(22);
    doc.setTextColor(40, 40, 40);
    doc.text("Medical Scan Report", 105, 20, { align: "center" });
    ...
    doc.setFontSize(10);
    doc.text(`Name: ${name || "N/A"}`, 14, 50);
    doc.text(`Email: ${email || "unknown"}`, 14, 60);

    // Add scan details
    doc.setFontSize(12);
    doc.setTextColor(100, 100, 100);
    doc.text("Scan Details", 14, 80);

    doc.setFontSize(10);
    doc.text(
      `Scan Date: ${dayjs(scanResult.createdAt).format("DD-MM-YYYY HH:mm")}`,
      14,
      90
    );
    doc.text(`Scan Type: ${scanResult.scanType || "X-Ray"}`, 14, 100);

    // Add analysis results
    doc.setFontSize(12);
    doc.setTextColor(100, 100, 100);
    doc.text("Analysis Results", 14, 120);

    doc.setFontSize(10);
    doc.text(`Result: ${scanResult.resultClass || "N/A"}`, 14, 130);
    doc.text(
      `Confidence: ${Math.round(scanResult.resultAccuracy * 100)}%`,
      14,
      140
    );
    ...
    // Save the PDF
    doc.save(`Scan_Report_${scanResult._id}.pdf`);
  };
